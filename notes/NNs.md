...
Batchnorm suffers when the batch size is small, e.g. computer vision, therefore there are variants of normalization, such as `instance normalization`, `layer normalization`, and `group normalization`[2]. `Layer normalization` is widely used in transformers.
